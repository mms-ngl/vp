    ...
    for batch_idx, batch in enumerate(train_loader):      
        ...
        if args.pro_bar:
            ...
            result_line = 'Epoch: [{0}][{1}/{2}]\t' \
                'Loss: {loss}, Avg. Loss: {avg_loss}, LR: {lr}\n'.format(
                    epoch, batch_idx, iterations,
                    loss=depth_loss.val, avg_loss=depth_loss.avg, lr=current_lr
                )
...
def validate(val_loader, model, criterion_d, device, epoch, args, log_dir):
    ...
    for batch_idx, batch in enumerate(val_loader):
        ...
        print("pred_crop: ", pred_crop)
        print("pred_crop.dtype: ", pred_crop.dtype)
        print("pred_crop.shape: ", pred_crop.shape)

        print("gt_crop: ", gt_crop)
        print("gt_crop.dtype: ", gt_crop.dtype)
        print("gt_crop.shape: ", gt_crop.shape)
